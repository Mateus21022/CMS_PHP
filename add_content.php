...
class add_content extends BancodeDados{
    ...
    public function addContent(){
        ...
        if (!empty($_POST['conteudo_adicionado'])&& isset($_SESSION['usuario_id'])){
       
            $conteudo_adicionado = $_POST['conteudo_adicionado'];
            $usuario_id = $_SESSION['usuario_id'];

  
            $operador = $connection->prepare('INSERT INTO conteudos_usuario (conteudo, usuario_id) VALUES (:conteudo_adicionado, :usuario_id)');
            $operador->bindParam(':conteudo_adicionado', $conteudo_adicionado);
            $operador->bindParam(':usuario_id', $usuario_id);
...
<form method = "POST">
    <input type = "text" name = "conteudo_adicionado" placeholder="Insira o conteudo que deseja adicionar" required> 
